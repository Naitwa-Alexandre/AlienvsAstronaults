...
canvas.addEventListener('click', function(){
    ...
    for (let i = 0; i < defenders.length; i++){
        if (defenders[i].x === gridPostionX && defenders[i].y === gridPostionY) return;
    }
...
function handleEnemies(){
    for (let i = 0; i < enemies.length; i++){
        ...
        if (enemies[i].health <= 0){
            let gainedResourcers = enemies[i].maxHealth/10;
            floatingMessages.push(new floatingMessage('+' + gainedResourcers, enemies[i].x, enemies[i].y, 30, 'black'));
            floatingMessages.push(new floatingMessage('+' + gainedResourcers, 250, 50, 30, 'gold'));
            numberOfResouces += gainedResourcers;
            score += gainedResourcers;
...
function handleGameStatus(){
    ...
    ctx.fillText('Resources: ' + numberOfResouces, 20, 80);
